...
# Name and look of this build. 'Hacker' is the NetGUIH2 terminal look;
# 'Standard' gives NetGUI's Light / Dark look.
$script:AppName = 'NetGUIH2'
$script:Skin = 'Hacker'
...
# Hacker skin: a marker in front of every log line
$script:LogMarkers = @{
    'LogSuccessColor' = '[+] '; 'LogErrorColor' = '[-] '; 'LogWarningColor' = '[!] '
    'LogInfoColor' = '[*] '; 'LogDebugColor' = '[.] '
}
...
function Write-Log {
    ...
    try {
        ...
        $marker = if ($script:Skin -ne 'Hacker') { '' } elseif ($key) { $script:LogMarkers[$key] } else { '[>] ' }
        $LogBox.AppendText('[' + [DateTime]::Now.ToString('HH:mm:ss') + '] ' + $marker + $Message + "`r`n")
        $LogBox.SelectionColor = $LogBox.ForeColor
        $LogBox.ScrollToCaret()
    } catch { }
}

function Format-NetGuiStatusText {
    <#
    .SYNOPSIS
        Status bar wording: plain (Standard skin) or terminal style (Hacker skin).
    .PARAMETER Kind
        Status (the left text), Link (connected to -Text), NoLink, Admin, User
    #>
    param([string]$Text, [ValidateSet('Status', 'Link', 'NoLink', 'Admin', 'User')][string]$Kind = 'Status')
    $hacker = $script:Skin -eq 'Hacker'
    switch ($Kind) {
        'Link' {
            if ($hacker) { if ($Text) { return 'LINK UP: ' + $Text.ToUpperInvariant() } else { return 'LINK UP' } }
            if ($Text) { return "Connected to $Text" } else { return 'Connected' }
        }
        'NoLink' { if ($hacker) { return 'LINK DOWN' } else { return 'Not connected' } }
        'Admin' { if ($hacker) { return 'PRIV: ADMIN' } else { return 'Administrator' } }
        'User' { if ($hacker) { return 'PRIV: USER' } else { return 'Standard user' } }
        default { if ($hacker) { return '> ' + $Text.ToUpperInvariant() } else { return $Text } }
    }
}

function Format-NetGuiCaption {
    # Hacker skin: captions in capitals; Standard skin: unchanged
    param([string]$Text)
    if ($script:Skin -ne 'Hacker' -or -not $Text) { return $Text }
    return $Text.ToUpperInvariant()
}

function New-NetGuiFont {
    <#
    .SYNOPSIS
        Segoe UI (Standard skin) or Consolas (Hacker skin); -Strong = semibold / bold.
        Consolas is set 8% smaller, so text takes about as much room as in Segoe UI
        and fits the same layout.
    #>
    param([single]$Size, [switch]$Strong)
    if ($script:Skin -eq 'Hacker') {
        $style = if ($Strong) { [System.Drawing.FontStyle]::Bold } else { [System.Drawing.FontStyle]::Regular }
        return New-Object System.Drawing.Font('Consolas', ([Math]::Round($Size * 0.92 * 4) / 4), $style)
    }
    $name = if ($Strong) { 'Segoe UI Semibold' } else { 'Segoe UI' }
...
function Set-NetGuiStatus {
    ...
    try {
        if ($Text) {
            $bar.StatusLabel.Text = Format-NetGuiStatusText -Text $Text
...
function Update-ConnectionStatus {
    ...
    if ($null -eq $bar) { return }
    if ($IsConnected) {
        $bar.ConnectionStatus.Text = Format-NetGuiStatusText -Text $ServerName -Kind Link
        $bar.ConnectionStatus.Tag = 'Success'
    } else {
        $bar.ConnectionStatus.Text = Format-NetGuiStatusText -Kind NoLink
...
function New-DashboardPanel {
    ...
    $lblTitle.Text = Format-NetGuiCaption -Text $Title
    $lblTitle.Tag = 'Muted'
...
function Get-NetGuiSettings {
    ...
    if ($settings.HackerTheme -notin $script:HackerThemeNames) { $settings.HackerTheme = 'Matrix' }
    return $settings
}
...
# Colour tokens per theme. Every control gets its colours from here, so the themes
# stay consistent (and View > Toggle Theme recolours everything). The Standard skin
# uses Light and Dark; the Hacker skin uses Matrix, Amber and Cyber.
$script:Themes = @{
    Light = @{
        Name = 'Light'
        ...
        DangerBack = '#C42B1C'; DangerHover = '#A8241A'; DangerPressed = '#8E1F16'; DangerText = '#FFFFFF'; DangerBorder = '#C42B1C'
    ...
    Dark = @{
        ...
        DangerBack = '#B8322A'; DangerHover = '#CC3F36'; DangerPressed = '#9E2A23'; DangerText = '#FFFFFF'; DangerBorder = '#B8322A'
    ...
    # Hacker skin: phosphor green on black
    Matrix = @{
        Name = 'Matrix'
        WindowBack = '#020602'; HeaderBack = '#000000'; CardBack = '#030B04'; CardBorder = '#0F5A21'
        Text = '#3DFF6E'; TextMuted = '#23A84C'
        Accent = '#00FF41'; AccentHover = '#66FF8C'; AccentPressed = '#00C232'; AccentText = '#001A07'
        NavText = '#23A84C'; NavSelected = '#00FF41'; NavHover = '#9DFFB6'
        ButtonBack = '#041207'; ButtonBorder = '#23A84C'; ButtonHover = '#0B2D14'; ButtonPressed = '#12421E'; ButtonText = '#3DFF6E'
        DangerBack = '#240404'; DangerHover = '#420909'; DangerPressed = '#5A0C0C'; DangerText = '#FF5555'; DangerBorder = '#FF5555'
        DisabledBack = '#020602'; DisabledBorder = '#0C3314'; DisabledText = '#1C6630'
        InputBack = '#000000'; InputText = '#3DFF6E'; LogBack = '#000000'
        SuccessText = '#00FF41'; WarningText = '#FFB000'; ErrorText = '#FF5555'
        SuccessBack = '#06290F'; WarningBack = '#2B1E00'; ErrorBack = '#2B0909'
        LogSuccessColor = '#00FF41'; LogErrorColor = '#FF5555'; LogWarningColor = '#FFB000'; LogInfoColor = '#00E5FF'; LogDebugColor = '#23A84C'
        TextBoxForeColor = '#3DFF6E'; StatusStripForeColor = '#23A84C'
    }
    # Hacker skin: amber monochrome terminal
    Amber = @{
        Name = 'Amber'
        WindowBack = '#070400'; HeaderBack = '#000000'; CardBack = '#0C0700'; CardBorder = '#5C3A00'
        Text = '#FFB000'; TextMuted = '#B57C00'
        Accent = '#FFB000'; AccentHover = '#FFC94D'; AccentPressed = '#CC8B00'; AccentText = '#1A0F00'
        NavText = '#B57C00'; NavSelected = '#FFC21A'; NavHover = '#FFDB85'
        ButtonBack = '#140C00'; ButtonBorder = '#B57C00'; ButtonHover = '#2B1B00'; ButtonPressed = '#3D2700'; ButtonText = '#FFB000'
        DangerBack = '#240404'; DangerHover = '#420909'; DangerPressed = '#5A0C0C'; DangerText = '#FF5A47'; DangerBorder = '#FF5A47'
        DisabledBack = '#070400'; DisabledBorder = '#2E1E00'; DisabledText = '#6B4A00'
        InputBack = '#000000'; InputText = '#FFB000'; LogBack = '#000000'
        SuccessText = '#FFE38A'; WarningText = '#FF7A00'; ErrorText = '#FF5A47'
        SuccessBack = '#2B2206'; WarningBack = '#2B1500'; ErrorBack = '#2B0909'
        LogSuccessColor = '#FFE38A'; LogErrorColor = '#FF5A47'; LogWarningColor = '#FF7A00'; LogInfoColor = '#FFC21A'; LogDebugColor = '#B57C00'
        TextBoxForeColor = '#FFB000'; StatusStripForeColor = '#B57C00'
    }
    # Hacker skin: cyan and magenta on deep blue-black
    Cyber = @{
        Name = 'Cyber'
        WindowBack = '#04040D'; HeaderBack = '#000007'; CardBack = '#070A19'; CardBorder = '#1D3363'
        Text = '#7DF9FF'; TextMuted = '#4A90B3'
        Accent = '#FF2BD6'; AccentHover = '#FF6BE3'; AccentPressed = '#C21FA3'; AccentText = '#14000F'
        NavText = '#4A90B3'; NavSelected = '#00F0FF'; NavHover = '#BDFCFF'
        ButtonBack = '#081128'; ButtonBorder = '#00B8D4'; ButtonHover = '#0F1E45'; ButtonPressed = '#162A5E'; ButtonText = '#7DF9FF'
        DangerBack = '#24040F'; DangerHover = '#42091D'; DangerPressed = '#5A0C27'; DangerText = '#FF4D6D'; DangerBorder = '#FF4D6D'
        DisabledBack = '#04040D'; DisabledBorder = '#15224A'; DisabledText = '#2E5470'
        InputBack = '#000000'; InputText = '#7DF9FF'; LogBack = '#000000'
        SuccessText = '#39FF88'; WarningText = '#FFD23F'; ErrorText = '#FF4D6D'
        SuccessBack = '#062914'; WarningBack = '#2B2406'; ErrorBack = '#2B0913'
        LogSuccessColor = '#39FF88'; LogErrorColor = '#FF4D6D'; LogWarningColor = '#FFD23F'; LogInfoColor = '#00F0FF'; LogDebugColor = '#4A90B3'
        TextBoxForeColor = '#7DF9FF'; StatusStripForeColor = '#4A90B3'
    }
}
$script:HackerThemeNames = @('Matrix', 'Amber', 'Cyber')
...
function Get-Theme {
    # A theme of the current skin (its first theme when the name is not one of them)
    param([string]$ThemeName)
    if ($script:Skin -eq 'Hacker') {
        if ($script:HackerThemeNames -contains $ThemeName) { return $script:Themes[$ThemeName] }
        return $script:Themes.Matrix
    }
    if ($ThemeName -eq 'Dark') { return $script:Themes.Dark }
    return $script:Themes.Light
}

function Get-NextThemeName {
    # View > Toggle Theme: Light <-> Dark, or Matrix -> Amber -> Cyber in the Hacker skin
    param([string]$Current)
    $order = if ($script:Skin -eq 'Hacker') { $script:HackerThemeNames } else { @('Light', 'Dark') }
    return $order[([Array]::IndexOf($order, $Current) + 1) % $order.Count]
}
...
function Set-NetGuiButtonStyle {
    ...
    switch ($role) {
        ...
        'Danger' { $colors = @($Theme.DangerBack, $Theme.DangerHover, $Theme.DangerPressed, $Theme.DangerText, $Theme.DangerBorder) }
...
# Hacker skin: cards also get bright corner brackets, like a head-up display.
$script:NetGuiBorderPainter = {
    ...
    if ($script:Skin -eq 'Hacker' -and $script:CurrentTheme -and [string]$control.Tag -eq 'Card') {
        $r = $control.Width - 1; $b = $control.Height - 1; $n = 12
        $corner = New-Object System.Drawing.Pen($script:CurrentTheme.NavSelected, 2)
        try {
            $g = $e.Graphics
            $g.DrawLine($corner, 0, 0, $n, 0); $g.DrawLine($corner, 0, 0, 0, $n)
            $g.DrawLine($corner, $r - $n, 0, $r, 0); $g.DrawLine($corner, $r, 0, $r, $n)
            $g.DrawLine($corner, 0, $b, $n, $b); $g.DrawLine($corner, 0, $b - $n, 0, $b)
            $g.DrawLine($corner, $r - $n, $b, $r, $b); $g.DrawLine($corner, $r, $b - $n, $r, $b)
...
function Convert-NetGuiLogColors {
    ...
    $rtf = $LogBox.Rtf
    # Two steps (colour -> placeholder -> new colour): a new colour that equals another
    # old one must not be replaced a second time
    $keys = @('LogSuccessColor', 'LogErrorColor', 'LogWarningColor', 'LogInfoColor', 'LogDebugColor', 'Text')
    for ($k = 0; $k -lt $keys.Count; $k++) {
        $old = $From[$keys[$k]]
        $rtf = $rtf.Replace(('\red{0}\green{1}\blue{2};' -f $old.R, $old.G, $old.B), "@@NETGUI$k@@")
    }
    for ($k = 0; $k -lt $keys.Count; $k++) {
        $new = $To[$keys[$k]]
        $rtf = $rtf.Replace("@@NETGUI$k@@", ('\red{0}\green{1}\blue{2};' -f $new.R, $new.G, $new.B))
    }
    $LogBox.Rtf = $rtf
    $LogBox.SelectionStart = $LogBox.TextLength
    $LogBox.ScrollToCaret()
    ...
    while ($stack.Count -gt 0) {
        ...
        try {
            switch ($c.GetType().Name) {
                ...
                'Button' {
                    Set-NetGuiButtonStyle -Button $c -Theme $Theme
                    if ($script:Skin -eq 'Hacker') { $c.Text = $c.Text.ToUpperInvariant() }
                }
                ...
                'PictureBox' { $c.BackColor = $Theme.CardBack }
...
function Start-SessionMonitor {
    ...
    $script:SessionTimer.Add_Tick({
        ...
        if ($idle -ge $script:SessionTimeoutMinutes) {
            ...
            [System.Windows.Forms.MessageBox]::Show("Session expired due to inactivity.`n`n$script:AppName will now close for security.", 'Session Timeout', 'OK', 'Warning') | Out-Null
            $this.Tag.Close()
        }
    })
...
function Set-NetGuiDialogStyle {
    ...
    param($Form)
    $Form.Font = if ($script:Fonts) { $script:Fonts.Normal } else { New-Object System.Drawing.Font('Segoe UI', 9) }
...
function Show-StartupPasswordDialog {
    ...
    $form.Text = if ($IsFirstRun) { "Set $script:AppName Startup Password" } else { "$script:AppName Authentication" }
    ...
    $lblTitle = New-Object System.Windows.Forms.Label
    $lblTitle.Text = if ($IsFirstRun) { "Welcome to $script:AppName" } else { 'Please authenticate to continue' }
    $lblTitle.Location = New-Object System.Drawing.Point(20, $y)
    $lblTitle.Size = New-Object System.Drawing.Size(450, 25)
    $lblTitle.Font = New-NetGuiFont 13 -Strong
    $lblTitle.Tag = 'Accent'
    ...
    if ($IsFirstRun) {
        ...
        $btnSetPassword.Add_Click({
            ...
            try {
                ...
                [System.Windows.Forms.MessageBox]::Show("Password set successfully!`n`n$script:AppName will now start.", 'Success', 'OK', 'Information') | Out-Null
        ...
        $lblLocked.Font = New-NetGuiFont 10 -Strong
        $lblLocked.Tag = 'Error'
        ...
        $lblPassword = New-Object System.Windows.Forms.Label
        $lblPassword.Text = "Enter your password to access $($script:AppName):"
        $lblPassword.Location = New-Object System.Drawing.Point(20, $y)
        $lblPassword.Size = New-Object System.Drawing.Size(450, 20)
        $form.Controls.Add($lblPassword)
        $y += 30
...
function Invoke-CommandRunner {
    ...
    $lblInfo.Font = New-NetGuiFont 11 -Strong
    $lblInfo.Tag = 'Accent'
...
function Show-AdminRequiredMessage {
    ...
    Write-Log -Message "Restart $script:AppName with `"Run as Administrator`" to use this tab (everything else works without it)" -Color 'Yellow' -LogBox $LogBox
    [System.Windows.Forms.MessageBox]::Show(
        "Changing adapter settings requires Administrator privileges.`n`nClose $script:AppName and start it with 'Run as Administrator' to use this tab. All other tabs work without it.",
...
$script:CurrentTheme = Get-Theme -ThemeName $(if ($script:Skin -eq 'Hacker') { $script:Settings.HackerTheme } else { $script:Settings.Theme })
...
# Shared fonts (one GDI object each instead of one per control); the skin picks the face
$script:Fonts = @{
    Normal      = New-NetGuiFont 9
    Semibold    = New-NetGuiFont 9 -Strong
    CardTitle   = New-NetGuiFont 10.5 -Strong
    Nav         = New-NetGuiFont 10
    NavSelected = New-NetGuiFont 10 -Strong
    StatValue   = New-NetGuiFont 18 -Strong
    Body        = New-NetGuiFont 10
    HelpTitle   = New-NetGuiFont 16 -Strong
    HelpHeading = New-NetGuiFont 12.5 -Strong
    HelpLabel   = New-NetGuiFont 10 -Strong
    Spacer      = New-NetGuiFont 5
    Mono        = New-Object System.Drawing.Font('Consolas', 9)
    MonoLarge   = New-Object System.Drawing.Font('Consolas', 10)
    Logo        = New-Object System.Drawing.Font('Consolas', 9, [System.Drawing.FontStyle]::Bold)
}
...
function New-NetGuiControl {
    ...
    # Hacker skin: button captions stay in capitals when the code changes them
    if ($Type -eq 'Button' -and $script:Skin -eq 'Hacker') {
        $c.Add_TextChanged({ $upper = $this.Text.ToUpperInvariant(); if ($this.Text -cne $upper) { $this.Text = $upper } })
    }
    if ($Bounds) {
...
function New-NetGuiCard {
    ...
    if ($Title) { [void](New-NetGuiControl Label @(15, 15) ([ordered]@{ Text = (Format-NetGuiCaption -Text $Title); AutoSize = $true; Font = $script:Fonts.CardTitle; Tag = 'CardTitle' }) $card) }
    return $card
}
...
function Show-NetGuiMessage {
    param([string]$Text, [string]$Title = $script:AppName, [string]$Icon = 'Information', [string]$Buttons = 'OK')
...
try {
    if ($script:RequireStartupPassword) {
        if (Test-StartupPasswordExists) {
            Write-SecurityAudit -Level Info -Event "$script:AppName startup" -Details 'Password authentication required'
            ...
            Write-SecurityAudit -Level Success -Event 'Authentication successful' -Details "$script:AppName starting"
        } else {
            ...
            if (-not (Show-StartupPasswordDialog -IsFirstRun)) {
                Show-NetGuiMessage -Text "Startup password is required for security.`n`n$script:AppName will now exit." -Title 'Password Required' -Icon Warning | Out-Null
                Write-SecurityAudit -Level Warning -Event 'First run - password not set' -Details 'Application exit'
                exit 0
            }
            Write-SecurityAudit -Level Success -Event 'First run - password configured' -Details "$script:AppName starting"
        }
    } else {
        Write-SecurityAudit -Level Info -Event "$script:AppName startup" -Details 'Team mode - no startup password'
    }
} catch {
    Show-NetGuiMessage -Text "Security initialization failed:`n`n$($_.Exception.Message)`n`n$script:AppName will now exit." -Title 'Security Error' -Icon Error | Out-Null
...
$mainForm.Text = if ($script:Skin -eq 'Hacker') { "$script:AppName :: network operations console" } else { "$script:AppName - Network Management Tool" }
...
$menuViewTheme.Add_Click({
    ...
    $newTheme = Get-NextThemeName -Current $previous.Name
    $script:CurrentTheme = Get-Theme -ThemeName $newTheme
    if ($script:Skin -eq 'Hacker') { $script:Settings.HackerTheme = $newTheme } else { $script:Settings.Theme = $newTheme }
...
$menuHelpAbout.Add_Click({
    ...
    Show-NetGuiMessage -Text $about -Title "About $script:AppName" | Out-Null
})
...
function New-NetGuiTab {
    ...
    $caption = Format-NetGuiCaption -Text $Text
    $width = [System.Windows.Forms.TextRenderer]::MeasureText($caption, $script:Fonts.NavSelected).Width + 28
    $navItem = New-NetGuiControl Label @($x, 0, $width, 39) ([ordered]@{ Text = $caption; TextAlign = 'MiddleCenter'; Font = $script:Fonts.Nav; Tag = 'NavItem'; Cursor = [System.Windows.Forms.Cursors]::Hand }) $script:navBar
...
# ============================================
# HACKER SKIN: DASHBOARD BANNER
# ============================================

$script:HackerLogo = @'
    _   ______________________  ________  _____
   / | / / ____/_  __/ ____/ / / /  _/ / / /__ \
  /  |/ / __/   / / / / __/ / / // // /_/ /__/ /
 / /|  / /___  / / / /_/ / /_/ // // __  // __/
/_/ |_/_____/ /_/  \____/\____/___/_/ /_//____/
'@
$script:CodeRainChars = '0123456789ABCDEF#$%&*+<=>?@'

function New-NetGuiHackerBanner {
    <#
    .SYNOPSIS
        Hacker skin: the logo, a blinking prompt and falling code at the top of the
        dashboard. The code only moves while the dashboard is on screen.
    #>
    param([int[]]$Bounds, $Parent)
    $card = New-NetGuiCard -Bounds $Bounds -Parent $Parent -Anchor 'Top,Left,Right'
    $logo = ($script:HackerLogo -replace "`r?`n", "`r`n")
    [void](New-NetGuiControl Label @(18, 12, 400, 74) ([ordered]@{ Text = $logo; Font = $script:Fonts.Logo; Tag = 'Accent' }) $card)
    [void](New-NetGuiControl Label @(18, 88, 400, 18) ([ordered]@{ Text = '// NETWORK OPERATIONS CONSOLE'; Font = $script:Fonts.Mono; Tag = 'Muted' }) $card)
    $user = if ($env:USERNAME) { $env:USERNAME.ToLowerInvariant() } else { 'user' }
    $machine = if ($env:COMPUTERNAME) { $env:COMPUTERNAME.ToLowerInvariant() } else { 'localhost' }
    $script:hackerPromptText = '{0}@{1}:~$ ./{2} --status' -f $user, $machine, $script:AppName.ToLowerInvariant()
    $script:hackerPrompt = New-NetGuiControl Label @(18, 110, 410, 18) ([ordered]@{ Text = $script:hackerPromptText + '_'; Font = $script:Fonts.Mono; Tag = 'Success'; AutoEllipsis = $true }) $card
    $script:hackerRain = New-NetGuiControl PictureBox @(440, 2, ($Bounds[2] - 443), ($Bounds[3] - 4)) ([ordered]@{ Anchor = 'Top,Bottom,Left,Right'; Tag = 'Rain' }) $card
    $script:hackerRainState = @{ Bitmap = $null; Drops = $null; Tick = 0; Random = (New-Object System.Random) }
    $script:hackerRainTimer = New-Object System.Windows.Forms.Timer
    $script:hackerRainTimer.Interval = 80
    # Decoration must never interrupt work: any drawing error just stops the rain
    $script:hackerRainTimer.Add_Tick({ try { Step-NetGuiCodeRain } catch { $this.Stop() } })
    $script:hackerRainTimer.Start()
}

function Step-NetGuiCodeRain {
    <#
    .SYNOPSIS
        One frame of the falling code: fade the picture a little, then draw one
        character per column. Skipped while the dashboard is hidden or minimised.
    #>
    $state = $script:hackerRainState
    $pic = $script:hackerRain
    if ($null -eq $pic -or $pic.IsDisposed) { return }
    $state.Tick++
    # Blinking cursor after the prompt (about twice a second)
    if ($state.Tick % 7 -eq 0) {
        $cursor = if ((($state.Tick / 7) % 2) -eq 0) { '_' } else { ' ' }
        $script:hackerPrompt.Text = $script:hackerPromptText + $cursor
    }
    if (-not $pic.Visible -or $mainForm.WindowState -eq [System.Windows.Forms.FormWindowState]::Minimized) { return }
    $w = $pic.ClientSize.Width
    $h = $pic.ClientSize.Height
    if ($w -lt 12 -or $h -lt 14) { return }
    $cellW = 12; $cellH = 14
    $theme = $script:CurrentTheme
    if ($null -eq $state.Bitmap -or $state.Bitmap.Width -ne $w -or $state.Bitmap.Height -ne $h) {
        $old = $state.Bitmap
        $state.Bitmap = New-Object System.Drawing.Bitmap($w, $h)
        $g = [System.Drawing.Graphics]::FromImage($state.Bitmap)
        try { $g.Clear($theme.CardBack) } finally { $g.Dispose() }
        $columns = [Math]::Max(1, [int][Math]::Floor($w / $cellW))
        $state.Drops = [int[]]::new($columns)
        for ($i = 0; $i -lt $columns; $i++) { $state.Drops[$i] = -$state.Random.Next(0, [Math]::Max(2, [int]($h / $cellH) * 2)) }
        $pic.Image = $state.Bitmap
        if ($old) { $old.Dispose() }
    }
    $rows = [int][Math]::Ceiling($h / $cellH)
    $chars = $script:CodeRainChars
    $g = [System.Drawing.Graphics]::FromImage($state.Bitmap)
    $fade = New-Object System.Drawing.SolidBrush([System.Drawing.Color]::FromArgb(60, $theme.CardBack))
    $headBrush = New-Object System.Drawing.SolidBrush($theme.NavHover)
    $tailBrush = New-Object System.Drawing.SolidBrush($theme.NavSelected)
    try {
        $g.TextRenderingHint = [System.Drawing.Text.TextRenderingHint]::AntiAlias
        $g.FillRectangle($fade, 0, 0, $w, $h)
        for ($i = 0; $i -lt $state.Drops.Count; $i++) {
            $row = $state.Drops[$i]
            if ($row -ge 0 -and $row -le $rows) {
                $x = $i * $cellW
                # the previous head turns from white to the theme colour, then fades
                if ($row -gt 0) { $g.DrawString([string]$chars[$state.Random.Next($chars.Length)], $script:Fonts.Mono, $tailBrush, $x, ($row - 1) * $cellH) }
                $g.DrawString([string]$chars[$state.Random.Next($chars.Length)], $script:Fonts.Mono, $headBrush, $x, $row * $cellH)
            }
            if ($row -gt $rows -and $state.Random.Next(0, 30) -eq 0) { $state.Drops[$i] = 0 } else { $state.Drops[$i] = $row + 1 }
        }
    } finally {
        $fade.Dispose(); $headBrush.Dispose(); $tailBrush.Dispose(); $g.Dispose()
    }
    $pic.Invalidate()
}
...
# Hacker skin: logo and code rain above the status cards
$dashTop = 16
if ($script:Skin -eq 'Hacker') {
    New-NetGuiHackerBanner -Bounds @(16, 16, 1148, 140) -Parent $tab0
    $dashTop = 168
}
# Four equal status cards that stretch with the window
$statsTable = New-NetGuiControl TableLayoutPanel @(10, $dashTop, 1160, 100) ([ordered]@{ ColumnCount = 4; RowCount = 1; Anchor = 'Top,Left,Right' }) $tab0
...
$activityCard = New-NetGuiCard -Bounds @(16, ($dashTop + 112), 1148, (482 - $dashTop)) -Title 'Recent Activity' -Parent $tab0 -Anchor 'Top,Bottom,Left,Right'
...
$btnExportDHCPWorkDir = New-NetGuiControl Button @(124, 12, 150, 28) ([ordered]@{ Text = 'Export to Working Dir'; Enabled = $false; Anchor = 'Top,Right' }) $dhcpLogCard
$btnExportDHCPFolder = New-NetGuiControl Button @(282, 12, 138, 28) ([ordered]@{ Text = 'Export to Folder...'; Enabled = $false; Anchor = 'Top,Right' }) $dhcpLogCard
...
[void](New-NetGuiControl Label @(264, 50, 216, 18) ([ordered]@{ Text = 'Favorites (right-click to add)'; Tag = 'Muted' }) $dnaFunctionsCard)
...
$script:HelpMarkup = @'
# {APP} Help
{APP} runs as a standard user. Only the Network Configuration tab needs "Run as Administrator" (it changes adapter IP settings).
...
- View > Toggle Theme (Ctrl+T) switches between {THEMES}
- The RDOX Exports tab exports embedded resource files
- Settings and caches are stored next to {APP}.ps1, or in %LOCALAPPDATA%\NetGUI when that folder is read-only
- The window size is saved when you close {APP}
'@
...
function Set-NetGuiHelpText {
    ...
    #>
    $theme = $script:CurrentTheme
    $themeNames = if ($script:Skin -eq 'Hacker') { 'the Matrix (green), Amber and Cyber (cyan) themes' } else { 'the Light and Dark themes' }
    $helpText.Clear()
    foreach ($markupLine in ($script:HelpMarkup -split "`r?`n")) {
        $raw = $markupLine.Replace('{APP}', $script:AppName).Replace('{THEMES}', $themeNames)
        $text = $raw; $font = $script:Fonts.Body; $color = $theme.Text
        $indent = 0; $hanging = 0; $bullet = $false; $gap = $false
        if ($raw.StartsWith('# ')) { $text = Format-NetGuiCaption -Text $raw.Substring(2); $font = $script:Fonts.HelpTitle; $color = $theme.NavSelected }
        elseif ($raw.StartsWith('## ')) { $text = Format-NetGuiCaption -Text $raw.Substring(3); $font = $script:Fonts.HelpHeading; $gap = $true }
...
$mainForm.Add_FormClosing({
    ...
    if ($script:hackerRainTimer) { $script:hackerRainTimer.Stop() }
    try {
